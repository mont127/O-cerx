...
rem ==========================================================
rem  O-cer v1.8 — VMware detection + auto-switch behavior
rem
rem  What changes vs v1.7:
rem   - Detects VMware (Fusion/Workstation/ESXi) from Windows (Manufacturer/Model)
rem   - If VMware detected:
rem       * Automatically disables bcdedit BootNext/bootsequence attempt
rem       * Prints VMware-specific next steps (Fusion ARM can't boot custom ESP entries)
rem
rem  Notes:
rem   - VMware Fusion on Apple Silicon typically only boots:
rem       Windows Boot Manager / Virtual CDROM / Network
rem     and does NOT offer "Boot from EFI file" for custom ESP paths.
rem ==========================================================

title O-cer Prep v1.8
...
if ($IsVmware) {
  Warn 'VMware detected. Auto-switching behavior: disabling bcdedit boot override (commonly unsupported in VMware firmware).' 
}
...
if ($IsVmware -and $tryBcd -eq 'YES') {
  Warn 'bcdedit bootsequence disabled on VMware (auto). Use the firmware boot menu / CDROM boot instead.'
  $tryBcd = 'NO'
}
...
try {
  ...
  try {
    ...
    if ($IsVmware) {
      Warn 'VMware note:'
      Warn ' - Many VMware firmwares (especially Fusion ARM) will NOT boot custom ESP entries like \\EFI\\ocer\\BOOTAA64.EFI.'
      Warn ' - To boot the installer in VMware, attach the ISO to the VM CD/DVD and boot "EFI VMware Virtual SATA CDROM".'
    } else {
      Info 'Next:'
      Info " - Reboot"
      Info " - Use firmware boot menu and select \\EFI\\ocer\\$bootName"
    }
