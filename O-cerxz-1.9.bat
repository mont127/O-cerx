...
function Detect-Vmware {
  try { $cs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction Stop; return (($cs.Manufacturer + ' ' + $cs.Model) -match 'VMware') } catch { return $false }
}
...
if ($IsVmware) { Warn 'VMware detected.' }
...
Write-Host ' - Mount ISO (you may see it appear/disappear; that is normal)'
