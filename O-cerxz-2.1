...
rem
rem  Restore mode:
rem   - Run: O-cerxz-2.1.bat /restore
rem   - Restores any backups created by strategy (2) or (3)
rem
rem  Notes:
rem   - Some firmwares (older ThinkPads like T450s) ignore BootNext and do not
rem     provide "Boot from EFI file". In those cases, only (2) or (3) can be
rem     made automatic.
rem   - Strategy (3) requires Secure Boot OFF.
rem ==========================================================

title O-cerxz Prep 2.1
...
try {
  ...
  if ($auto -eq '3') {
    # Swap Windows Boot Manager to force firmware to load our EFI (works even on locked OEM, BUT invasive)
    $winBoot = Join-Path $espMount 'EFI\Microsoft\Boot\bootmgfw.efi'
    ...
    Warn 'To restore: boot back into Windows and run: O-cerxz-2.1.bat /restore'
  }
