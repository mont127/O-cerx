...
rem ==========================================================
rem  O-cerxz 2.0.1 (single .bat)
rem
rem  Fix for: "Add-PartitionAccessPath ... access path is already in use"
rem   - We no longer rely on drive letters to mount the ESP for staging.
rem   - ESP is mounted to a dedicated folder: %ProgramData%\Ocer\mnt\esp
rem   - This avoids collisions with mapped drives, SUBST, hidden mounts, etc.
rem
rem  Still supports:
...
function Mount-PartitionToFolder([Microsoft.Management.Infrastructure.CimInstance]$part, [string]$folder) {
  ...
  # If something is already mounted there, detach it
  try {
...
function Get-FreeLetterAll {
  # True free letter check (volumes + PSDrives + existing drives)
  $used = New-Object System.Collections.Generic.HashSet[string]
  try {
    foreach ($d in [System.IO.DriveInfo]::GetDrives()) { $null = $used.Add($d.Name.Substring(0,1).ToUpper()) }
  } catch {}
  try {
    foreach ($pd in (Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue)) { $null = $used.Add($pd.Name.ToUpper()) }
  } catch {}
  # avoid A/B by convention
...
function Test-UefiEvidence {
  ...
  # 3) ESP exists on OS disk and has bootmgfw.efi
  try {
